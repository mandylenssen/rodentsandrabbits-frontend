            ...
            {visibleEntries && visibleEntries.length > 0 ? (
                <>
                    {visibleEntries.map((log, index) => (
                        <div key={index}>
                            <section className="logbook-card-wrapper">
                                <label className="logbook-name-date-label">
                                    <p><b>{log.petsIds.map(petId => pets[petId]?.name || "Unknown Pet").join(", ")}</b></p>
                                    <p>{new Date(log.date).toLocaleDateString()}</p>
                                </label>
                                <article className="logbook-card-content"><div className="logbook-image-wrapper">
                                    {imageUrls[log.id] && <img className="logbook-image" src={imageUrls[log.id]} alt="Log" />}
                                </div>

                            <p>{log.entry}</p>

                        </article> <div className="logbook-squiggle-image"></div></section></div>
                    ))}
